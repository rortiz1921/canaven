  ...
  const generatePDF=()=>{
    ...
    try{
      ...
      while(cur<=to){
        ...
        if(loaded>0){const dd=cur.getDate(),mm=cur.getMonth()+1,yy=String(cur.getFullYear()).slice(2);days.push({label:dd+"/"+mm+"/"+yy,val:loaded});}
      ...
      Object.entries(schedules).forEach(([date,ds])=>{
        ...
        if(!mmap[key])mmap[key]={label:MONTHS_ES[d.getMonth()].slice(0,3)+" "+String(d.getFullYear()).slice(2),val:0};
      ...
      const vData=vehicles.map(v=>{
        const ld=Object.entries(schedules).filter(([date])=>{const d=new Date(date+"T12:00:00");return d>=from&&d<=to;})
      ...
      const fromLbl=fmt(from);const toLbl=fmt(to);
      const tableRows=days.map(d=>'<tr><td style="border:1px solid #ccc;padding:5px 14px;text-align:center;">'+d.label+'</td><td style="border:1px solid #ccc;padding:5px 14px;text-align:center;font-weight:bold;">'+d.val+'</td></tr>').join('');
      const availRows=vData.map(v=>{const bW2=Math.round(v.pct*1.8);const bc=v.pct>=20?"#4ade80":v.pct>=10?"#b5c832":"#1a56c4";return '<tr><td style="padding:4px 8px;font-weight:bold;font-size:10px;">'+v.plate+'</td><td style="padding:4px 8px;"><svg width="180" height="12" xmlns="http://www.w3.org/2000/svg"><rect x="0" y="0" width="180" height="12" fill="#eee" rx="3"/><rect x="0" y="0" width="'+bW2+'" height="12" fill="'+bc+'" rx="3"/></svg></td><td style="padding:4px 8px;font-size:10px;color:#1a56c4;font-weight:bold;">'+v.val+' cargues</td><td style="padding:4px 8px;font-size:11px;font-weight:900;color:#b5c832;">'+v.pct+'%</td></tr>';}).join('');
      const obsLines=(pdfObs||('Promedio de cargues por dia: '+(days.length>0?(totalDays/days.length).toFixed(1):0)+' viajes.')).split('\n').map(l=>'<div style="margin:3px 0;">- '+l+'</div>').join('');
      const html='<!DOCTYPE html><html><head><meta charset="utf-8"><style>@media print{body{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important;}@page{margin:1.5cm;size:A4;}.no-print{display:none!important;}}body{font-family:Arial,sans-serif;margin:0;padding:28px;color:#222;font-size:11px;}.header{display:flex;justify-content:space-between;align-items:center;border-bottom:2px solid #1a56c4;padding-bottom:10px;margin-bottom:18px;}.chart-wrap{border:1px solid #ddd;border-radius:4px;padding:14px;margin:10px 0;}.chart-title{text-align:center;font-weight:bold;font-size:12px;margin-bottom:8px;}.sec{font-weight:bold;font-size:11px;margin:14px 0 5px;}table{border-collapse:collapse;}th{background:#1a56c4;color:#fff;padding:5px 20px;font-size:11px;}tfoot td{font-weight:bold;background:#f0f0f0;border:1px solid #ccc;padding:5px 14px;text-align:center;}</style></head><body>'
        +'<div class="no-print" style="position:fixed;top:10px;right:10px;"><button onclick="window.print()" style="background:#1a56c4;color:#fff;border:none;border-radius:7px;padding:9px 16px;font-size:13px;font-weight:700;cursor:pointer;">🖨️ Guardar PDF</button></div>'
        +'<div class="header"><img src="'+LOGO_B64+'" style="height:48px;object-fit:contain;"/><div style="text-align:right;font-size:10px;color:#555;">Informe generado: '+today+'</div></div>'
        +'<h2 style="text-align:center;font-size:13px;letter-spacing:1px;margin:0 0 14px;">INFORME OPERATIVO SEMANAL</h2>'
        +'<p><strong>1. FECHA:</strong> '+today+'</p>'
        +'<p class="sec">2. ACTIVIDADES:</p><div style="margin-left:18px;line-height:1.8;"><div>1. Programación de vehículos diarios.</div><div>2. Seguimiento de GPS para asegurar cantidad de vehículos programados</div><div>3. Solicitud de documentos operativos a los propietarios</div><div>4. Comunicación con personal de Canacol para asegurar la actividad y cumplir la programación.</div><div>5. Reunión propietarios temas de facturación y pagos.</div></div>'
        +'<p class="sec">3. INDICADORES: cargue de vehículos del '+fromLbl+' al '+toLbl+'</p>'
        +'<div class="chart-wrap"><div class="chart-title">CARGUES POR DÍA</div>'+svgDay+'<div style="text-align:center;margin-top:4px;font-size:9px;color:#555;">■ CARGUES</div></div>'
        +'<p class="sec">4. COMPARATIVO MENSUAL DE CARGUES</p>'
        +'<div class="chart-wrap"><div class="chart-title">CARGUES POR MES</div>'+svgMonth+'<div style="text-align:center;margin-top:4px;font-size:9px;color:#555;">■ TOTAL CARGUES POR MES</div></div>'
        +'<p class="sec">5. INDICADOR: DEL '+fromLbl.toUpperCase()+' A '+toLbl.toUpperCase()+'</p>'
        +'<table style="margin:0 auto;"><thead><tr><th>DIA</th><th>VIAJES</th></tr></thead><tbody>'+tableRows+'</tbody><tfoot><tr><td>TOTAL</td><td>'+totalDays+'</td></tr></tfoot></table>'
        +'<p class="sec">6. DISPONIBILIDAD POR VEHÍCULO</p>'
        +'<div class="chart-wrap"><div class="chart-title">% Participación de Cargues por Vehículo</div><table style="width:100%;"><tbody>'+availRows+'</tbody><tfoot><tr><td colspan="4" style="padding:5px 8px;font-size:10px;color:#666;border-top:1px solid #ddd;">Total cargues del período: <strong>'+totalDays+'</strong></td></tr></tfoot></table></div>'
        +'<p class="sec">7. OBSERVACIONES:</p>'+obsLines
        +'<div style="border-top:1px dashed #aaa;margin-top:28px;padding-top:10px;"><div style="margin-bottom:4px;font-size:11px;">Realizo:</div><img src="'+FIRMA_B64+'" style="height:55px;object-fit:contain;display:block;"/><div><strong>RONALD JESUS ORTIZ</strong></div></div>'
        +'</body></html>';
      const blob=new Blob([html],{type:"text/html;charset=utf-8;"});
      const url=URL.createObjectURL(blob);
      const a=document.createElement("a");
      a.href=url;a.download="Informe_Canaven_"+pdfFrom+"_"+pdfTo+".html";
          ...
          <div>
            ...
            <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:"14px",marginBottom:14}}>
              ...
              <button onClick={generatePDF} disabled={generatingPdf} className="tap"
                ...
                <span style={{fontSize:18}}>📄</span>{generatingPdf?"Generando...":"⬇️ Descargar Informe"}
              </button>
            </div>
            {/* KPIs */}
