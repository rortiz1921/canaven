...
].map(plate=>({plate, available, owner:"", replacedBy}));
...
function load(key,fb){try{const v=localStorage.getItem(key);return v?JSON.parse(v);}catch{return fb;}}
...
const badge = (ok)=>({display:"inline-flex",alignItems:"center",gap:4,padding:"3px 9px",borderRadius:20,fontSize:10,fontWeight:700,letterSpacing:"0.07em",textTransform:"uppercase",background?C.green+"22".red+"22",color?C.green.red,border:1px solid ${ok?C.green+"55":C.red+"55"}});
...
setSyncing(true);
try{await setDoc(doc(db,"canaven","data"),{vehicles,schedules,updatedAt Date().toISOString()});}
...
const nv=[...vehicles,{plate,available,owner:"",replacedBy}];
updateVehicles(nv);
document.getElementById("new-plate-input").value="";
};
const removeVehicle=(plate)=>updateVehicles(vehicles.filter(v=>v.plate!==plate));
const toggleAvailability=(plate)=>updateVehicles(vehicles.map(v=>v.plate===plate?{...v,available:!v.available}));

const saveEditVehicle=()=>{
if(!editVehicle)return;
const newPlate=(editVehicle.newPlate||"").trim().toUpperCase()||editVehicle.plate;
const nv=vehicles.map(v=>v.plate===editVehicle.plate?{...v,plate,owner.owner});
// also update schedules if plate changed
if(newPlate!==editVehicle.plate){
const ns={};
Object.entries(schedules).forEach(([date,ds])=>{ns[date]=ds.map(s=>s.plate===editVehicle.plate?{...s,plate});});
...
let nv=vehicles.map(v=>v.plate===originalPlate?{...v,available,replacedBy});
if(!nv.find(v=>v.plate===newPlate)){
const idx=nv.findIndex(v=>v.plate===originalPlate);
nv.splice(idx+1,0,{plate,available,owner:"",replacedBy});
}
updateVehicles([...nv]);
...
const nv=vehicles.filter(p=>p.plate!==v.replacedBy).map(p=>p.plate===originalPlate?{...p,available,replacedBy});
updateVehicles(nv);
};
...
.map(plate=>({plate,loaded,loadedTime,loadDate:"",guia:"",volumen:"",observaciones:""}));
...
day[idx]={...day[idx],loaded,loadedTime,loadDate:"",guia:"",volumen:""};
updateSchedules({...schedules,[selectedDate]});
} else {
setLoadForm({fecha(),guia:"",volumen:""});
...
if(idx===-1)return;
day[idx]={...day[idx],loaded,loadedTime Date().toLocaleTimeString("es-CO",{hour:"2-digit",minute:"2-digit"}),
loadDate.fecha,guia.guia,volumen.volumen};
updateSchedules({...schedules,[selectedDate]});
...
const weeklyData=useMemo(()=>weekDates.map(date=>{const ds=schedules[date]||[];const d=new Date(date+"T12:00:00");return{name,programados.length,cargados.filter(v=>v.loaded).length};}), [schedules,weekDates]);
const monthlyData=useMemo(()=>{const byDay={};Object.entries(schedules).forEach(([date,ds])=>{const d=new Date(date+"T12:00:00");if(d.getMonth()===currentMonth&&d.getFullYear()===currentYear)byDay[d.getDate()]={name:${d.getDate()},programados.length,cargados.filter(v=>v.loaded).length};});return Object.values(byDay).sort((a,b)=>parseInt(a.name)-parseInt(b.name));},[schedules,currentMonth,currentYear]);
...
const reportData=reportView==="weekly"?weeklyData;
...
const gap = (cW - bWdata.length) / (data.length+1);
return (
<svg width={W} height={H} style={{overflow:"visible"}}>
{[0,0.5,1].map(p => {
const v2=Math.round(maxVp); const y=PT+cH-Math.round(pcH);
...
<img src={"data/png;base64,"} style={{height:48,objectFit:"contain"}}/>
...
{vData.map((v,i) => {
const bc = v.pct>=20?"#4ade80".pct>=10?"#b5c832":"#1a56c4";
return (
...
<div style={{width:${v.pct}%,height:"100%",background,borderRadius:3}}/>
</div>
...
<div style={{background.card,border:1px solid ${C.border},borderRadius:20,padding:"32px 24px",textAlign:"center"}}>
...
<span style={{background.blue+"22",color.blue,border:1px solid ${C.blue}44,borderRadius:20,padding:"3px 10px",fontSize:11,fontWeight:700}}>⚙️ Admin</span>
<span style={{background.green+"22",color.green,border:1px solid ${C.green}44,borderRadius:20,padding:"3px 10px",fontSize:11,fontWeight:700}}>👁️ Visitante</span>
</div>
<div style={{display:"flex",justifyContent:"center",gap:12,marginBottom:24}}>
{[0,1,2,3].map(i=>(<div key={i} style={{width:14,height:14,borderRadius:"50%",background.length>i?C.blue.border,transition:"background 0.15s"}}/>))}
</div>
...
style={{background===""?"transparent"?C.red+"22".surface,border:1px solid ${k===""?"transparent":pinError?C.red+"55":C.border},borderRadius:12,padding:"16px 0",fontSize:20,fontWeight:700,color==="⌫"?C.muted.text,cursor===""?"default":"pointer"}} disabled={k===""}>
{k}
</button>
))}
</div>
{pinError&&<div style={{color:C.red,fontSize:13,fontWeight:600,marginBottom:12}}>PIN incorrecto.</div>}
<button onClick={handleLogin} disabled={pinInput.length<4} style={{width:"100%",background.length===4?C.blue.border,color.length===4?"#fff".muted,border:"none",borderRadius:12,padding:"14px 0",fontSize:16,fontWeight:700,cursor.length===4?"pointer":"not-allowed"}}>Ingresar</button>
</div>
<div style={{textAlign:"center",marginTop:20,fontSize:11,color.muted}}>CANAVEN Transportes de Colombia SAS</div>
</div>
</div>
);
...
return(
<div style={{minHeight:"100vh",background.bg,color.text,fontFamily:"'Segoe UI',Arial,sans-serif",paddingBottom:80}}>
