...
export default function App() {
  ...
  const [loadForm, setLoadForm] = useState({fecha:"", guia:"", volumen:"", numero:""});
  ...
  const addSelectedToSchedule=()=>{
    ...
    const toAdd=selectedPlates.filter(plate=>!existing.find(e=>e.plate===plate))
      .map(plate=>({plate,loaded:false,loadedTime:null,loadDate:"",guia:"",volumen:"",volumenGov:"",cargueNumero:"",observaciones:""}));
  ...
  const openConfirmLoad=(plate)=>{
    ...
    if(existing?.loaded){
      ...
      day[idx]={...day[idx],loaded:false,loadedTime:null,loadDate:"",guia:"",volumen:"",volumenGov:"",cargueNumero:""};
      updateSchedules({...schedules,[selectedDate]:day});
    } else {
      setLoadForm({fecha:getTodayStr(),guia:"",volumen:"",numero:""});
      setOcrMessage("");
  ...
  const loadTesseract = () => new Promise((resolve,reject)=>{
    ...
    if(existing){existing.addEventListener("load",()=>resolve(window.Tesseract),{once:true});existing.addEventListener("error",()=>reject(new Error("No se pudo cargar el lector OCR.")),{once:true});return;}
    const script=document.createElement("script");
    script.src="https://cdn.jsdelivr.net/npm/tesseract.js@5/dist/tesseract.min.js";
    script.async=true;
    script.dataset.canavenTesseract="1";
    script.onload=()=>window.Tesseract?resolve(window.Tesseract):reject(new Error("OCR no disponible."));
    script.onerror=()=>reject(new Error("No se pudo cargar el lector OCR. Verifica tu conexión a Internet."));
    document.head.appendChild(script);
  });

  const parseOcrLoadData = (text) => {
    const clean=(text||"").replace(/\u00a0/g," ").replace(/\r/g,"\n");
    const lines=clean.split("\n").map(x=>x.trim()).filter(Boolean);
    let guia="", volumen="", numero="";
    const guidePatterns=[
      /(?:NO\.?|N\.°|N°|NUMERO|NÚMERO)\s*(?:DE\s*)?(?:GUIA|GU[IÍ]A|REMESA)\s*[:#-]?\s*([A-Z0-9-]{3,30})/i,
      /(?:GUIA|GU[IÍ]A|REMESA)\s*(?:NO\.?|N\.°|N°|NUMERO|NÚMERO)?\s*[:#-]?\s*([A-Z0-9-]{3,30})/i
    ];
    for(const re of guidePatterns){const m=clean.match(re);if(m){guia=m[1].trim();break;}}
    const volPatterns=[
      /(?:VOLUMEN|VOL\.?)\s*(?:GOV)?\s*[:#-]?\s*([0-9]{1,3}(?:[.,][0-9]{1,3})?(?:[.,][0-9]{3})*)\s*(?:BARRILES?|BBL|BLS?)?/i,
      /([0-9]{1,3}(?:[.,][0-9]{1,3})?(?:[.,][0-9]{3})*)\s*(?:BARRILES?|BBL|BLS?)\s*(?:GOV)?/i,
      /(?:GOV)\s*[:#-]?\s*([0-9]{1,3}(?:[.,][0-9]{1,3})?(?:[.,][0-9]{3})*)/i
    ];
    for(const re of volPatterns){const m=clean.match(re);if(m){volumen=m[1].trim();break;}}
    const numPatterns=[
      /(?:N[°º\.\s]*|NUMERO)\s*(?:DE\s*)?(?:CARGUE|CARGA|CUPON|CUPÓN)\s*[:#-]?\s*([A-Z0-9-]{1,30})/i,
      /(?:CARGUE|CARGA)\s*(?:N[°º\.\s]*|NUMERO)?\s*[:#-]?\s*([A-Z0-9-]{1,30})/i
    ];
    for(const re of numPatterns){const m=clean.match(re);if(m){numero=m[1].trim();break;}}
    if(!guia){const l=lines.find(x=>/(GU[IÍ]A|REMESA)/i.test(x));if(l){const m=l.match(/[A-Z0-9-]{3,30}/i);if(m)guia=m[0];}}
    return {numero,guia,volumen};
  };

  const extractLoadDataFromImage = async (file) => {
    if(!file)return;
    setOcrLoading(true);setOcrMessage("Leyendo imagen… puede tardar unos segundos.");
    try{
      const Tesseract=await loadTesseract();
      const result=await Tesseract.recognize(file,"spa",{logger:m=>{
        if(m.status==="recognizing text" && typeof m.progress==="number") setOcrMessage(`Leyendo imagen… ${Math.round(m.progress*100)}%`);
      }});
      const parsed=parseOcrLoadData(result.data?.text||"");
      setLoadForm(prev=>({...prev,numero:parsed.numero||prev.numero,guia:parsed.guia||prev.guia,volumen:parsed.volumen||prev.volumen}));
      setOcrMessage(parsed.numero||parsed.guia||parsed.volumen?"Datos detectados. Revísalos antes de confirmar.":"No pude identificar los datos automáticamente. Puedes ingresarlos manualmente.");
    }catch(e){console.error("OCR error:",e);setOcrMessage("No fue posible leer la imagen. Ingresa los datos manualmente.");}
    finally{setOcrLoading(false);}
  };

  const confirmLoad=()=>{
    if(!confirmModal)return;
    if(!loadForm.numero?.trim() || !loadForm.volumen?.trim()){setOcrMessage("Completa N° de cargue y Volumen GOV antes de confirmar.");return;}
    const day=[...(schedules[selectedDate]||[])];
    const idx=day.findIndex(v=>v.plate===confirmModal.plate);if(idx===-1)return;
    day[idx]={...day[idx],loaded:true,loadedTime:new Date().toLocaleTimeString("es-CO",{hour:"2-digit",minute:"2-digit"}),loadDate:loadForm.fecha,guia:loadForm.guia,volumen:loadForm.volumen,volumenGov:loadForm.volumen,cargueNumero:loadForm.numero};
    updateSchedules({...schedules,[selectedDate]:day});setConfirmModal(null);setOcrMessage("");
  };
  ...
  const buildWhatsAppText=()=>{
    ...
    scheduleForDate.forEach((v,i)=>{
      const status=v.loaded?`✅ Cargado${v.loadedTime?" · "+v.loadedTime:""}${v.guia?" · Guía: "+v.guia:""}${v.cargueNumero?" · N°: "+v.cargueNumero:""}${v.volumenGov?" · GOV: "+v.volumenGov+" bbl":""}`:` ⏳ Programado`;
    ...
    filtered.forEach(date=>{
      ...
      (schedules[date]||[]).forEach(v=>{
        if(v.loaded){
          ...
          rows.push([date,dayName,v.plate,veh.owner||"",v.loaded?"Cargado":"Pendiente",v.loadedTime||"",v.loadDate||"",v.guia||"",v.volumenGov||v.volumen||"",v.observaciones||""]);
        }
      });
    });
  ...
  const generatePDF = async () => {
    setGeneratingPdf(true);setShowPdfPreview(true);
    try{
      await new Promise(r=>setTimeout(r,500));
      const el=pdfRef.current;if(!el)throw new Error("No se pudo preparar el informe para PDF.");
      ...
      const ratio=pdfW/canvas.width,scaledH=canvas.height*ratio;let y=0,remaining=scaledH;
      while(remaining>0){pdf.addImage(imgData,"PNG",0,-y,pdfW,scaledH);remaining-=pdfH;y+=pdfH;if(remaining>0)pdf.addPage();}
      pdf.save("Informe_Canaven_"+pdfFrom+"_"+pdfTo+".pdf");
    }catch(e){console.error("PDF error:",e);alert("No se pudo generar el PDF: "+(e?.message||e));}
    finally{setGeneratingPdf(false);setShowPdfPreview(false);}
  };
      ...
      {confirmModal&&(
        <div style={{position:"fixed",inset:0,background:"#000a",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
          <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:24,width:"100%",maxWidth:360}}>
            ...
            <div style={{marginBottom:12}}><div style={{fontSize:11,color:C.muted,marginBottom:4}}>Fecha de cargue</div><input type="date" value={loadForm.fecha} onChange={e=>setLoadForm(p=>({...p,fecha:e.target.value}))} style={{width:"100%",background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:"9px 12px",color:C.text,fontSize:13,outline:"none"}}/></div>
            <div style={{marginBottom:12}}><div style={{fontSize:11,color:C.muted,marginBottom:4}}>N° de cargue</div><input value={loadForm.numero||""} onChange={e=>setLoadForm(p=>({...p,numero:e.target.value}))} placeholder="Ej: 12345" style={{width:"100%",background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:"9px 12px",color:C.text,fontSize:13,outline:"none"}}/></div>
            <div style={{marginBottom:12}}><div style={{fontSize:11,color:C.muted,marginBottom:4}}>No. Guía</div><input value={loadForm.guia||""} onChange={e=>setLoadForm(p=>({...p,guia:e.target.value}))} placeholder="Opcional" style={{width:"100%",background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:"9px 12px",color:C.text,fontSize:13,outline:"none"}}/></div>
            <div style={{marginBottom:12}}><div style={{fontSize:11,color:C.muted,marginBottom:4}}>Volumen GOV (barriles)</div><input type="text" inputMode="decimal" value={loadForm.volumen||""} onChange={e=>setLoadForm(p=>({...p,volumen:e.target.value}))} placeholder="Ej: 12.500" style={{width:"100%",background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:"9px 12px",color:C.text,fontSize:13,outline:"none"}}/></div>
            <div style={{marginBottom:12}}><input ref={ocrInputRef} type="file" accept="image/*" style={{display:"none"}} onChange={e=>{const f=e.target.files?.[0];if(f)extractLoadDataFromImage(f);e.target.value="";}}/><button type="button" onClick={()=>ocrInputRef.current?.click()} disabled={ocrLoading} style={{width:"100%",background:C.surface,border:`1px solid ${C.blue}`,borderRadius:9,padding:"10px 0",color:C.blue,cursor:ocrLoading?"wait":"pointer",fontWeight:700,fontSize:12}}>{ocrLoading?"🔎 Leyendo imagen…":"📷 Leer datos desde imagen"}</button>{ocrMessage&&<div style={{fontSize:10,color:C.muted,marginTop:6,lineHeight:1.4}}>{ocrMessage}</div>}</div>
